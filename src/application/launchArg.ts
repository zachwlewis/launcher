...
export abstract class LaunchArg {
  ...
  get type(): LaunchArgType | string {
    return this._props.type;
  }
  get display(): LaunchArgDisplay {
    return this._props.display || LaunchArgDisplay.Default;
  }

  /** The tooltip to display when hovering over the argument. */
  get tooltip(): string {
    return this._props.tooltip || '';
  }

  /** Does the argument have a tooltip to display? */
  get hasTooltip(): boolean {
    return this.tooltip !== undefined && this.tooltip.length > 0;
  }
  ...
  get pre(): string {
    return `${this._props.pre != null ? this._props.pre : ''}`;
  }
  get post(): string {
    return `${this._props.post != null ? this._props.post : ''}`;
  }
  ...
  toString(): string {
    return `${this.pre}${this.stringValue}${this.post}`;
  }
...
class LaunchArgBoolean extends LaunchArg {
  ...
  get stringValue(): string {
    const vs = (this._props as LaunchArgBooleanProps).values || [
      'false',
      'true',
    ];
...
export class LaunchArgOption extends LaunchArg {
  ...
  get stringValue(): string {
    ...
    console.log(index);
    return (this.props as LaunchArgOptionProps).options[
...
export function constructLaunchArg(
  props: LaunchArgProps,
  value?: LaunchArgValue,
): LaunchArg | null {
  ...
  if (props.type == LaunchArgType.Boolean)
    return new LaunchArgBoolean(
      { ...LaunchArgBooleanDefaults, ...props },
      value,
    );
