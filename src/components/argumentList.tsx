...
export class ArgumentList extends Component<ApplicationListProps> {
  ...
  makeArgumentItem(arg: AT.AnyArg, index: number): JSX.Element {
    switch (arg.type) {
      case 'string':
        return this.buildStringArgumentItem(arg, index);
      case 'number':
        return this.buildIntArgumentItem(arg, index);
      case 'boolean':
        return this.buildBooleanArgumentItem(arg, index);
      case 'option':
        return this.buildOptionArgumentItem(arg, index);
    }
  ...
  render() {
    const listItems = this.props.definitions.map((arg, index) =>
      this.makeArgumentItem(arg, index),
    );
